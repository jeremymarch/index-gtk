...
sauros_add_related (MYSQL *conn, int index_id, int related_id)
{
  ...
  /* make sure both terms are preferred */
  snprintf(queryBuffer, MAX_QUERY_LEN, 
          "SELECT preferred "
          "FROM index_terms "
          "WHERE index_id IN (%i, %i) AND preferred = 0",
           index_id, related_id);
...
/* non-preferred terms can't have parents or children */
  if (!preferred && parent_id)
  {
    ...
    return -1;
               ...
               "(index_id, term, parent_id, preferred, updated_by, "
  ...
  end = int10_to_str(last_id, end, 10);
  end = strmov(end, ", '");
  ...
  if (mysql_real_query(conn, queryBuffer, end - queryBuffer) != 0)
  {
    snprintf(errorBuffer, MAX_ERROR_LEN, "query: %s\nError: %s", 
             queryBuffer, mysql_error(conn));
    return -1;
  }
 ...
 */
  ...
  if ((res_set = mysql_store_result(conn)) == NULL)
  {
    snprintf(errorBuffer, MAX_ERROR_LEN, "Delete failed: couldn't store results");
    return -1;
  }
...
sauros_select_term (MYSQL *conn, int index_id, indexForm *index)
{
  ...
  snprintf(queryBuffer, MAX_QUERY_LEN, 
           "SELECT a.index_id, a.term, b.see_also, b.scope_note, a.updated, "
           "a.created, a.preferred, a.parent_id "
